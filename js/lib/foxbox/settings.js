define(['exports', './common/event-dispatcher'], function (exports, _eventDispatcher) {
  'use strict';
  ...
  class Settings extends _eventDispatcher2.default {
    constructor() {
      super(['session', 'polling-setting']);

      const localOrigin = storage.getItem(`${ PREFIX }localOrigin`);

      const pollingEnabled = storage.getItem(`${ PREFIX }pollingEnabled`) !== null ? storage.getItem(`${ PREFIX }pollingEnabled`) === 'true' : DEFAULT_POLLING_ENABLED;

      this._configured = storage.getItem(`${ PREFIX }configured`) !== null ? storage.getItem(`${ PREFIX }configured`) === 'true' : false;
    ...
    get registrationService() {
      return localStorage.registrationServer || REGISTRATION_SERVICE;
    }
