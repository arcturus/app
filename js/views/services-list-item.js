define(['exports', 'components/react'], function (exports, _react) {
  ...
  class ServicesListItem extends _react2.default.Component {
    constructor(props) {
      super(props);
    ...
    componentDidMount() {
      switch (this.service.type) {
        case 'light':
          this.service.isAvailable().then(available => {
            this.setState({ available });
          }).catch(console.error.bind(console));

          this.service.isOn().then(on => {
            this.setState({ on });
          }).catch(console.error.bind(console));
          break;
        case 'motion-sensor':
          this.service.isMotionDetected().then(this.onMotion);
          this.service.on('motion', this.onMotion);
          break;
        case 'door-lock':
          this.service.isLocked().then(locked => {
    ...
    handleLightOnChange(evt) {
      ...
      this.service.turn(on).catch(error => {
    ...
    onDoorLockUnlock(evt) {
      ...
      this.service.lockUnlock(locked).catch(error => {
        // Revert back to the previous value.
    ...
    renderLightService() {
      ...
      return _react2.default.createElement(
        'li',
        { className: 'service-list__item', 'data-icon': icon,
        ...
        _react2.default.createElement('div', { className: 'service-list__item-color-picker',
          style: { background: this.getBulbColour() } }),
        _react2.default.createElement(
          'label',
          null,
          _react2.default.createElement('input', { className: 'service-list__on-off-toggle', type: 'checkbox',
    ...
    renderDoorLock() {
      const doorLockNameNode = this.service.name ? _react2.default.createElement(
        'small',
        null,
        ` (${ this.service.name })`
      ) : null;
      ...
      return _react2.default.createElement(
        'li',
        { className: 'service-list__item', 'data-icon': 'door-lock' },
        _react2.default.createElement(
          'a',
          { className: 'service-list__item-link',
            href: `#services/${ this.service.id }` },
    ...
    renderMotionSensor() {
      const motionSensorNameNode = this.service.name ? _react2.default.createElement(
        'small',
        null,
        ` (${ this.service.name })`
      ) : null;
      ...
      return _react2.default.createElement(
        'li',
        { className: motionSensorClassName },
        _react2.default.createElement(
          'a',
          { className: 'service-list__item-link',
            href: `#services/${ this.service.id }` },
    ...
    renderGenericService(type = 'Unknown service', icon = 'unknown') {
      const serviceNameNode = this.service.name ? _react2.default.createElement(
        'small',
        null,
        ` (${ this.service.name })`
      ) : null;
      ...
      return _react2.default.createElement(
        'li',
        { className: 'service-list__item', 'data-icon': icon, 'data-connected': 'true' },
        _react2.default.createElement(
          'a',
          { className: 'service-list__item-link',
            href: `#services/${ this.service.id }` },
