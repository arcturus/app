define(['exports', './common/event-dispatcher', './common/sequential-timer', './services/base', './services/ip-camera', './services/light', './services/door-lock', './services/motion-sensor'], function (exports, _eventDispatcher, _sequentialTimer, _base, _ipCamera, _light, _doorLock, _motionSensor) {
  'use strict';
  ...
  var _sequentialTimer2 = _interopRequireDefault(_sequentialTimer);

  var _base2 = _interopRequireDefault(_base);

  var _ipCamera2 = _interopRequireDefault(_ipCamera);
  ...
  class Services extends _eventDispatcher2.default {
    constructor(db, api, settings) {
      ...
      this[p.cache] = null;
      this[p.pollingTimer] = new _sequentialTimer2.default(this[p.settings].pollingInterval);
    ...
    [p.getCache]() {
      ...
      return this[p.db].getServices().then(dbServices => {
     ...
     */
    [p.updateServiceList]() {
      return Promise.all([this[p.api].get('services'), this[p.db].getServices(), this[p.getCache]()]).then(([fetchedServices, storedServices, cache]) => {
        let servicesToAddCount = 0;
        fetchedServices.forEach(fetchedService => {
          const storedService = storedServices.find(storedService => storedService.id === fetchedService.id);
        ...
        const servicesToRemoveCount = storedServices.length + servicesToAddCount - fetchedServices.length;
        if (servicesToRemoveCount > 0) {
          storedServices.forEach(storedService => {
            const fetchedService = fetchedServices.find(fetchedService => fetchedService.id === storedService.id);
    ...
    [p.getServiceInstance](data) {
      switch (data.adapter) {
        case 'ip-camera@link.mozilla.org':
          return new _ipCamera2.default(data, this[p.api]);

        case 'philips_hue@link.mozilla.org':
          return new _light2.default(data, this[p.api]);

        case 'OpenZwave Adapter':
          if (this[p.hasDoorLockChannel](data.getters) || this[p.hasDoorLockChannel](data.setters)) {
            return new _doorLock2.default(data, this[p.api]);
          }

          if (Object.keys(data.getters).length > 0) {
            return new _motionSensor2.default(data, this[p.api]);
          }

          return new _base2.default(data, this[p.api]);
    ...
    [p.hasDoorLockChannel](channels) {
      return Object.keys(channels).find(key => channels[key].kind === 'DoorLocked');
    }
  }
