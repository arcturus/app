define(['exports', './common/event-dispatcher', './common/sequential-timer'], function (exports, _eventDispatcher, _sequentialTimer) {
  ...
  class Network extends _eventDispatcher2.default {
    ...
    init() {
      ...
      this[p.localPingTimer] = new _sequentialTimer2.default(pingInterval);
      this[p.localPingTimer].start(this[p.pingLocalBox]);

      this[p.remotePingTimer] = new _sequentialTimer2.default(pingInterval);
    ...
    fetchJSON(url, method = 'GET', body = undefined) {
      return this[p.fetch](url, 'application/json', method, body).then(response => response.json());
    }
    ...
    fetchBlob(url, blobType, method, body) {
      return this[p.fetch](url, blobType, method, body).then(response => response.blob());
    }
    ...
    [p.pingLocalBox]() {
      ...
      return this[p.ping](`${ this.localOrigin }/ping`).then(isOnline => this[p.onPong](p.local, isOnline));
    }
    ...
    [p.pingRemoteBox]() {
      ...
      return this[p.ping](`${ this.tunnelOrigin }/ping`).then(isOnline => this[p.onPong](p.remote, isOnline));
    }
     ...
     */
    [p.ping](url) {
      return fetch(url, { cache: 'no-store' }).then(res => res.ok).catch(error => {
