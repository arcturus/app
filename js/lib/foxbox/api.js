define(['exports', './common/event-dispatcher', './common/sequential-timer'], function (exports, _eventDispatcher, _sequentialTimer) {
  'use strict';
  ...
  class API {
    constructor(net, settings) {
      ...
      this[p.watchTimer] = new _sequentialTimer2.default(this[p.settings].watchInterval);
      this[p.watchEventBus] = new _eventDispatcher2.default();
     ...
     */
    get(path) {
      return this[p.onceReady]().then(() => this[p.net].fetchJSON(this[p.getURL](path)));
    }
     ...
     */
    post(path, body) {
      return this[p.onceReady]().then(() => this[p.net].fetchJSON(this[p.getURL](path), 'POST', body));
    }
     ...
     */
    put(path, body) {
      return this[p.onceReady]().then(() => this[p.net].fetchJSON(this[p.getURL](path), 'PUT', body));
    }
    ...
    blob(path, body, accept = 'image/jpeg') {
      return this[p.onceReady]().then(() => {
        if (body) {
          return this[p.net].fetchBlob(this[p.getURL](path), accept, 'PUT', body);
        }
    ...
    [p.fetchGetterValues]() {
      ...
      const selectors = Array.from(this[p.watchGetters].values()).map(({ id }) => ({ id }));

      return this.put('channels/get', selectors).then(response => {
    ...
    [p.updateGetterValue](getter, getterValue) {
      ...
      if (!getterValue || !getter.value) {
        ...
        if (valueKind === 'Error') {
          console.error('Failed to retrieve value for getter (%s): %o', getter.id, getterValue[valueKind]);
